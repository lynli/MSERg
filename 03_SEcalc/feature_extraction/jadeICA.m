function [B,Bpca,scales] =  jadeICA(X,varargin)
%   B = jadeRmod(X, m) is an m*n matrix such that Y=B*X are separated sources
%    extracted from the n*T data matrix X.
%   If m is omitted,  components retaining 99.99% of variance are returned
...
%     rows of S=B*X.
%
%   B = jadeRmod(X, m, alreadywhitened,B) uses the variable 'alreadywhitened' to
%   tell jade not to rerun whitening (via PCA). This may be done in cases
%   where the function 'whiten' has already been run on 'X' prior to being input
%   to jadeR. 'B' is the PCA matrix calculated via 'whiten' which MUST be
%   input to jadeRmod in such a case.
%
...
% - Subtraction of mean via 'bsxfun' for better memory.
% - Automatic calculation of #PCs (prior to calculation of ICA), which retain 99.99% of
% the variance from the original data. This has been done so that no imaginary components are
% calculated via ICA. Additionally, retaining ALL components will probably
% lead to a lot of noise being introduced, as components which contribute
% the remainder 0.01% (over the 99.99% retained) are probably crap. Hence
% if 'm' is unspecified above, this code will calculate the appropriate
% number of #PCs (and hence #ICs) accordingly.
% - Input variable 'alreadywhitened'
%
%   SEE ALSO: WHITEN, jadeR
%
...
if(nargin == 1)
    fprintf('jade -> Number of output dimensions to be determined automatically.\n');    
    alreadywhitened = 0;
    m=[];
elseif(nargin == 2)
    m       = varargin{1};
    ...
    fprintf('jade -> Data already whitened\n');        
    m       = n;
    alreadywhitened  = varargin{2};
    B = varargin{3};
end

% Checking number of sources requested
if m>n
    fprintf('jade -> Do not ask more sources than sensors. Resetting to automatic determination\n'), 
    m=[];
    alreadywhitened = 0;
end

% to do: add a warning about complex signals
% Edit (Satish): has this been circumvented by retaining only significant PCs?
...
if ~alreadywhitened
    ...
    if ~exist('m','var') || isempty(m)
        PCs = n - find(varscaptured<0.999)+1; %PCs which retain 99.99% of variance        
        m = length(PCs);
        if m==0, 
            m=1; 
            PCs = 1;
